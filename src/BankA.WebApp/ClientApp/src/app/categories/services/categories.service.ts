...
export class CategoryService {
	...
	public async getList(search: CategoryPagedListRequest): Promise<PagedList<Category>> {
		let url = `${this.baseUrl}/categories?pageIndex=${search.PageIndex}&search=${search.Search}`;
		if (search.PageSize)
	...
	public async getParentList(): Promise<PagedList<Category>> {
		const url = `${this.baseUrl}/categories?pageIndex=0&&pageSize=100&isParent=true`;
		const resp = this.http.get<any>(url);
