...
export class RuleEditComponent extends BaseComponent implements OnInit {
	ruleId: number = 0;
	categoryLookup: any[] = [];
	...
	async save() {
		try {
			...
			await this.ruleService.update(this.ruleId, ruleReq);
	...
	async delete() {
		try {
			const confirm = await this.dialogService.showConfirmation('Would you like to delete this Rule?');
			if (confirm) {
				await this.ruleService.delete(this.ruleId);
				this.dialogRef.close(true);
			}
		} catch (error: any) {
			this.errorMessage = error;
	...
	private async loadRule() {
		try {
			this.loading = true;
			const result = await this.ruleService.get(this.ruleId);

			this.form.controls['RuleId'].setValue(result.RuleId);
			this.form.controls['Keywords'].setValue(result.Keywords);
			this.form.controls['CategoryId'].setValue({ Id: result.CategoryId, CategoryName: result.CategoryFullName }); 
			this.form.controls['MerchantName'].setValue(result.MerchantName);
